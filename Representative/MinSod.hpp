//  MinSod class, part of the SPARE library.
...
/** @brief File MinSod.hpp, containing the MinSod class.
 *
 * The file contains the MinSod class, implementing a cluster model based on
 * the Sum Of Distances (SOD) minimizing sample.
 *
 * @file MinSod.hpp
...
/** @brief Cluster model based on Sum Of Distances (SOD) minimization.
 *
 * This class models the @a Representative concept. The cluster representation approach is
 ...
 */
template <typename SampleType, typename Dissimilarity>
class MinSod
{
public:
...
// OPERATIONS
   ...
   RealType             Diss(const SampleType& rSample) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("MinSod, 0, Uninitialized object.");
                              }
    ...
    * @return The dissimilarity value.
    */
   RealType             Diss(const MinSod<SampleType, Dissimilarity>& rOther) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("MinSod, 1, Uninitialized object.");
                              }
...
// ACCESS
   ...
   /** Read-only access to the current max sod element index
    *
    * @return A reference to the max sod element index
    */
   const SampleSizeType& GetMaxSodIndex() const    { return mMaxSodIndex; }
   ...
   const SampleType&    getRepresentativeSample() const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("MinSod, 2, Uninitialized object.");
                              }

                              return mSamples[mMinSodIndex];
                           }
  // ALESSIO HACK FOR SPARE 1.x.
  // In SPARE 1.0 name "GetMinSodSample" was used. In SPARE 1.3 "getRepresentativeSample" was used.
  // However, they both refer to the same function. Double declaration hack, in order to improve compatibility. 
   const SampleType&    GetMinSodSample() const 
                             {
                                if ( mSamples.empty() )
                                {
                                   throw SpareLogicError("MinSod, 2, Uninitialized object.");
   ...
   /**
    * Read-only access to the internal dissimilarity matrix
    */
   const BoostRealSymmMatrix& DissimilarityMatrix() { return mDissMatrix; }
...
private:
   ...
   // Indice dell'elemento in mSamples che massimizza la SOD.
   SampleSizeType       mMaxSodIndex;
   ...
   void serialize(Archive & ar, const unsigned int version)
   {
      ...
      for (i= 0; i < k; i++)
      {
         for (j= 0; j <= i; j++)
         {
            ar & boost::serialization::make_nvp("dmel", mDissMatrix(i, j));
         }
      }

      ar & BOOST_SERIALIZATION_NVP(mSods);
      ar & BOOST_SERIALIZATION_NVP(mMinSodIndex);
      ar & BOOST_SERIALIZATION_NVP(mDiscardIndex);
   }  // BOOST SERIALIZATION
...
template <typename SampleType, typename Dissimilarity>
void
MinSod<SampleType, Dissimilarity>::Update(const SampleType& rSample)
{
   // Typedef locali.
   ...
   std::vector<RealType>::iterator                    Sit, S2it;
   ...
   //update the max sod
   S2it=std::max_element(mSods.begin(), mSods.end());

   mMaxSodIndex=boost::numeric::converter<SampleSizeType, SodDiffType>::convert(std::distance(mSods.begin(), S2it));

   // Aggiorno  WorstIndex, il peggiore &egrave; il pi&ugrave; distante dal rappresentante tra due scelti
   // a caso.
   k= boost::numeric::converter<BoostRealSymmMatrix::size_type, SampleSizeType>
...
template <typename SampleType, typename Dissimilarity>
void
MinSod<SampleType, Dissimilarity>::Init(NaturalType aM)
{
   // Variabili.
   ...
   if (mM < 1)
   {
      throw SpareLogicError("MinSod, 3, M cannot be less than 1.");
   }
   ...
   {
      throw SpareLogicError("MinSod, 4, Too large M value.");
   }
