//  PFuzzyMinSod class, part of the SPARE library.
...
/** @brief File PFuzzyMinSod.hpp, containing the PFuzzyMinSod class.
 *
 * The file contains the Probabilistic FuzzyMinSod class, implementing a cluster model based on
 ...
 * The replacement policy is fully probabilistic, selecting the element to be replaced according to its distance with respect to the MinSOD element.
 *
 * @file PFuzzyMinSod.hpp
 * @author Lorenzo Livi
 */

#ifndef _PFuzzyMinSod_h_
#define _PFuzzyMinSod_h_
 ...
 * The replacement policy is fully probabilistic, selecting the element to be replaced according to its distance to the MinSOD element.
...
// PUBLIC TYPES
   ...
   /**
    * Type for the samples membership values container
    */
   typedef std::vector<RealType> MembershipValuesType;
...
// OPERATIONS
   ...
   RealType             Diss(const SampleType& rSample) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("PFuzzyMinSod, 0, Uninitialized object.");
                              }
    ...
    * @return The dissimilarity value.
    */
   RealType             Diss(const PFuzzyMinSod<SampleType, Dissimilarity, Evaluator>& rOther) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("PFuzzyMinSod, 1, Uninitialized object.");
                              }
       ...
       RealType            Eval(const SampleType& rSample) const
                             {
                                if ( mSamples.empty() )
                                {
                                   throw SpareLogicError("PFuzzyMinSod, 4, Uninitialized object.");
                                }
...
// ACCESS
    ...
    */
   const MembershipValuesType& GetMemberships() const { return mMembershipValues; }
   ...
   const SampleType&    getRepresentativeSample() const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("PFuzzyMinSod, 2, Uninitialized object.");
                              }
...
private:
   ...
   MembershipValuesType mMembershipValues;
...
template <typename SampleType, typename Dissimilarity, typename Evaluator>
void
PFuzzyMinSod<SampleType, Dissimilarity, Evaluator>::Update(const SampleType& rSample)
{
    // Typedef locali.
    ...
    std::vector<RealType> probs;
    for(unsigned int h=0;h<mSamples.size();h++)
        probs.push_back((RealType)mDissMatrix(mMinSodIndex, h)/(RealType)mSods[mMinSodIndex]);
...
template <typename SampleType, typename Dissimilarity, typename Evaluator>
void
PFuzzyMinSod<SampleType, Dissimilarity, Evaluator>::Init(NaturalType aM)
{
   // Variabili.
   ...
   if (mM < 1)
   {
      throw SpareLogicError("PFuzzyMinSod, 3, M cannot be less than 1.");
   }
   ...
   {
      throw SpareLogicError("PFuzzyMinSod, 4, Too large M value.");
   }
