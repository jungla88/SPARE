//  RFFuzzyMinSod class, part of the SPARE library.
...
/** @brief File RFFuzzyMinSod.hpp, containing the FuzzyMinSod class.
 *
 * The file contains the RFFuzzyMinSod class, implementing a cluster model based on
 ...
 * The replacement policy has a deterministic behaviour, discarding the element farthest away from the current representative element.
 *
 * @file RFFuzzyMinSod.hpp
 * @author Lorenzo Livi
 */

#ifndef _RFFuzzyMinSod_h_
#define _RFFuzzyMinSod_h_
...
// OPERATIONS
   ...
   RealType             Diss(const SampleType& rSample) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("RFFuzzyMinSod, 0, Uninitialized object.");
                              }
    ...
    * @return The dissimilarity value.
    */
   RealType             Diss(const RFFuzzyMinSod<SampleType, Dissimilarity, Evaluator>& rOther) const
                           {
                              if ( mSamples.empty() )
                              {
                                 throw SpareLogicError("RFFuzzyMinSod, 1, Uninitialized object.");
                              }
       ...
       RealType            Eval(const SampleType& rSample) const
                             {
                                if ( mSamples.empty() )
                                {
                                   throw SpareLogicError("RFFuzzyMinSod, 4, Uninitialized object.");
                                }
...
template <typename SampleType, typename Dissimilarity, typename Evaluator>
void
RFFuzzyMinSod<SampleType, Dissimilarity, Evaluator>::Update(const SampleType& rSample)
{
   // Typedef locali.
   ...
   RealType max=std::numeric_limits<RealType>::min();
   for(unsigned int j=0;j<mSods.size();j++)
   {
       if(max<mSods[j])
       {
           max=mSods[j];
...
template <typename SampleType, typename Dissimilarity, typename Evaluator>
void
RFFuzzyMinSod<SampleType, Dissimilarity, Evaluator>::Init(NaturalType aM)
{
   // Variabili.
