//  IdVectorGraph class, part of the SPARE library.
...
/** @brief DOT file reader for the IdVectorGraph graph type.
 *
 * @file IdVectorGraph.hpp
 * @author Lorenzo Livi
 */

#ifndef IDVECTORGRAPHREADER_HPP
#define IDVECTORGRAPHREADER_HPP
...
// SPARE INCLUDES
#include <spare/Graph/Graph/IdVectorGraph.hpp>
...
/** @brief DOT file reader for (reals) vector-labeled graphs.
 */
class IdVectorGraphReader {
private:
...
public:
     ...
     */
    void read(std::ifstream& in, IdVectorGraph& g) const;
     ...
     * @param[out] g A compatible boost graph defined as a labeled graph with std::vector<spare::RealType>-valued labels
     */
    void read(std::string& in, IdVectorGraph& g) const;
};


void IdVectorGraphReader::read(std::ifstream& in, IdVectorGraph& g) const
{
    ...
    while(vi!=viEnd)
    {
        ...
        boost::get(v_info_t(), g, source)= tmpWeight;
...
void IdVectorGraphReader::read(std::string& in, IdVectorGraph& g) const
{
...
void IdVectorGraphReader::string2vector(std::string& s, std::vector<RealType>& v) const
{
    int cutAt;
