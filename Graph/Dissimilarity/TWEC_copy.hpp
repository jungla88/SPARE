...
//    template<class BoostGraphType >
//    RealType Diss_(const BoostGraphType& g1, const BoostGraphType& g2) const;
...
private:
    ...
    /**
     * Seed variable for RNG. If not set, default constructor use time(null)
     */
...
RealType TWEC<VerticesDiss, EdgesDiss>::Diss(const BoostGraphType& g1, const BoostGraphType& g2) const
{
    ...
    do{
        ...
        RealType Diss = Diss_<BoostGraphType>(g1,g2);

        if(Diss<dissMinTot){
            dissMinTot=Diss;
        }
        if(boost::num_vertices(g1)<2)
            break;
        shuffleCount++;
...
template <class VerticesDiss, class EdgesDiss>
template <class BoostGraphType >
RealType TWEC<VerticesDiss, EdgesDiss>::Diss_(const BoostGraphType& g1, const BoostGraphType& g2) const
{
...
//        itG1++;
    ...
    CND=( (orderG1+orderG2)-(2*std::min(orderG1, orderG2)) )/(orderG1+orderG2); // max(o1,o2) ?
