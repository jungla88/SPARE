//  TensorProduct class, part of the SPARE library.
...
/** @brief File TensorProduct.hpp, that contains the tensor product class.
 *
 * Contains the declaration of the TensorProduct class.
 *
...
/** @brief Tensor product computation for labeled graphs.
 *
...
public:
    ...
    /**
     * Funzione temporanea per ovviare al problema con la classe MCGraphCoverage
     */
    void productNT(const VectorGraph* g1, const VectorGraph* g2, RealType* weight) const;
...
RealType TensorProduct<KernelType, MatrixRepresentation>::product(const GraphType& g1, const GraphType& g2) const
{
    ...
    {
        for(NaturalType colG1=0; colG1<o1; colG1++)
        {
            if(m1(rowG1, colG1))
            {
                //data of g2
 ...
 */
template <class KernelType, class MatrixRepresentation>
void TensorProduct<KernelType, MatrixRepresentation>::productNT(const VectorGraph* g1,
        const VectorGraph* g2, RealType* weight) const
{
...
template <class KernelType, class MatrixRepresentation>
template <typename GraphType>
void TensorProduct<KernelType, MatrixRepresentation>::product(const GraphType* g1,
...
template <class KernelType, class MatrixRepresentation>
template <typename GraphType>
RealType TensorProduct<KernelType, MatrixRepresentation>::product(const GraphType& g1, const GraphType& g2,
        boost::numeric::ublas::matrix<RealType>& mOut) const
{
    NaturalType o1 = boost::num_vertices(g1);
    NaturalType o2 = boost::num_vertices(g2);
            ...
            if(m1(rowG1, colG1))
            {
                ...
                for(NaturalType rowG2=0; rowG2<o2; rowG2++)
                {
                    for(NaturalType colG2=0; colG2<o2; colG2++)
                    {
                        ...
                        //positions in the adjacency matrix of the product graph
